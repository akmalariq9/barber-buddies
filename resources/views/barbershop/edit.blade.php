...
<body class="h-screen flex items-center justify-center bg-cover" style="background-image: url('{{ asset("storage/aboutus.png") }}');">

    {{-- <h1 class="text-white text-2xl mb-6">Edit {{ $barberShop->name }} - Barber Shop Profile</h1> --}}

    <form action="{{ route('barbershop.update', $barberShop) }}" method="POST" class="bg-inherit p-6 rounded-lg shadow-md w-full max-w-md">
        @csrf
        @method('PUT')
